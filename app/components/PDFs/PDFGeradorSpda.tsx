...
interface PDFGeradorSpdaProps {
  cmsData: Cm[];
}

const PDFGeradorSpda: React.FC<PDFGeradorSpdaProps> = ({ cmsData }) => {
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      const addPageHeader = (doc: any, pageNum: number) => {
        ...
        doc.text("Relatório de Sensores SPDA", pageWidth / 2, titleY, {
          align: "center",
        });
      ...
      if (cmsData.length === 0) {
        ...
        yOffset += lineHeight;
        pdf.text(`Total de Sensores SPDA: ${totalSensores}`, margin, yOffset);
        yOffset += lineHeight;
        pdf.text(
          ...
          if (cm.equipamentos && cm.equipamentos.length > 0) {
            ...
            cm.equipamentos.forEach((eq) => {
              ...
              if (eq.sensores && eq.sensores.length > 0) {
                ...
                pdf.setFontSize(10);
                pdf.text(
                  `        Sensores SPDA (${eq.sensores.length}):`,
                  margin + 15,
                  yOffset
                );
                yOffset += lineHeight;
      ...
      pdf.save("relatorio_sensores_spda.pdf");
