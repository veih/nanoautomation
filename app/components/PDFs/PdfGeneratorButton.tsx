...
const PdfGeneratorButton: React.FC<PdfGeneratorButtonProps> = ({ cmsData }) => {
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      const addPageHeader = (doc: any, pageNum: number) => {
        ...
        doc.text("Relatório dos Equipamentos", pageWidth / 2, titleY, {
          align: "center",
        });
      ...
      const totalAtuadores = cmsData.reduce(
        (acc, cm) =>
          acc +
          (cm.equipamentos?.reduce(
            (eqAcc, eq) => eqAcc + (eq.atuadores?.length || 0),
            0
          ) || 0),
        0
      );
      ...
      if (cmsData.length === 0) {
        ...
        cmsData.forEach((cm, index) => {
          ...
          yOffset += lineHeight;
          pdf.text(
            `   Total de Atuadores: ${totalAtuadoresCm}`,
            margin,
            yOffset
          );
          yOffset += lineHeight;
          ...
          if (cm.equipamentos && cm.equipamentos.length > 0) {
            ...
            cm.equipamentos.forEach((eq) => {
              ...
              if (eq.descricao) {
                pdf.setFontSize(9);
                pdf.text(`        ${eq.descricao}`, margin + 15, yOffset);
                yOffset += lineHeight;
              }

              // Detalhes dos Atuadores
              if (eq.atuadores && eq.atuadores.length > 0) {
                pdf.setFont("helvetica", "bold");
                pdf.setFontSize(10);
                pdf.text(
                  `        Atuadores (${eq.atuadores.length}):`,
                  margin + 15,
                  yOffset
                );
                yOffset += lineHeight;

                eq.atuadores.forEach((atuador) => {
                  if (yOffset > pageHeight - 100) {
                    pdf.addPage();
                  ...
                  } else if (atuador.estado === AtuadorStatus.MANUTENCAO) {
                    statusText = "MANUTENÇÃO";
                    textColor = [255, 165, 0]; // Laranja
                  } else if (atuador.estado === AtuadorStatus.DESCONHECIDO) {
                  ...
                  if (
                    ...
                    const defectText = pdf.splitTextToSize(
                      `            Descrição do Defeito: ${atuador.descricaoDefeito}`,
